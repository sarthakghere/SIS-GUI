...
public class ViewCourse extends JFrame {
    ...
    public ViewCourse(String username) {
        ...
        try (Connection c = SQL.makeConnection();
            ...
            JDialog infoDialog = new JDialog(this, "All Course Information", true);
            ...
            JLabel removeLabel = new JLabel("Remove Course:");
            removeDropdown = new JComboBox<>(getCourseNames());
            JButton removeButton = new JButton("Remove");

            removeButton.addActionListener(new ActionListener() {
                @Override
                public void actionPerformed(ActionEvent e) {
                    removeCourse((String) removeDropdown.getSelectedItem());
    ...
    private void removeCourse(String courseName) {
        try (Connection c = SQL.makeConnection();
             PreparedStatement ps = c.prepareStatement("DELETE FROM course WHERE course_name = ?")) {
            ps.setString(1, courseName);
            int rowsAffected = ps.executeUpdate();
            if (rowsAffected > 0) {
                JOptionPane.showMessageDialog(this, "Course Removed Successfully", "Success", JOptionPane.INFORMATION_MESSAGE);
            } else {
                JOptionPane.showMessageDialog(this, "Course Not Removed", "Failure", JOptionPane.ERROR_MESSAGE);
            }
        } catch (Exception ex) {
    ...
    private Vector<String> getCourseNames() {
        ...
        try (Connection c = SQL.makeConnection();
            ...
            JOptionPane.showMessageDialog(this, "SQL Error while fetching course names: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(this, "Error while fetching course names: " + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
        }
